...
# Immediate thread unload (experimental)

`thread/unload` immediately releases one idle, unsubscribed thread runtime after graceful shutdown and rollout flush. Enable `capabilities.experimentalApi` and unsubscribe the current connection before calling it. The durable rollout is preserved, active or subscribed threads are left unchanged, and the operation does not cascade to spawned descendants.

The response status is `unloaded`, `notLoaded`, `active`, `hasSubscribers`, or `unloading`. Shutdown submission failures and timeouts are returned as JSON-RPC errors. A successful unload runs normal shutdown and root-only `SessionEnd` hooks, closes the rollout writer, emits `thread/status/changed` with `notLoaded`, emits `thread/closed`, and then returns. A later `thread/resume` reconstructs the durable history.
