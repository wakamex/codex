...
fn tool_summary_covers_supported_response_tool_items() {
    ...
    assert_eq!(
        summarize_tool_activity("turn-1", &history),
...
fn tool_summary_and_utf8_truncation_obey_hard_limits() {
    let short_history = (0..40)
        ...
        .collect::<Vec<_>>();
    let short_summary = summarize_tool_activity("turn-1", &short_history);
    ...
    let long_history = (0..40)
        ...
        .collect::<Vec<_>>();
    let summary = summarize_tool_activity("turn-1", &long_history);
